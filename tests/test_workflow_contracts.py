...
def test_run_request_preserves_identity_relationships():
    request = RunRequest(
        ...
        case_id="case_1",
        message_id="msg_1",
        trusted_jurisdiction={"province": "四川省", "city": "成都市"},
...
def test_workflow_state_json_round_trip():
    state = WorkflowState(
        ...
        case_facts={"province": "四川省", "disability_grade": "十级", "monthly_wage": 6000},
        intent_result={
...
def test_workflow_contracts_do_not_import_agent_framework_types():
    ...
    assert "from_agent_context" not in source
